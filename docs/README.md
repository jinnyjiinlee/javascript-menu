...
- [X] 프로그램 소개를 하는 메시지를 출력한다.
  - [X] 메시지: '`점심 메뉴 추천을 시작합니다.`'

코치 이름

입력

- [X] 사용자에게 코치 이름 입력을 받는다.
  - [X] 메시지: ‘코치의 이름을 입력해 주세요. (, 로 구분)’

내부 기능
- [X] 코치 이름을 컴마로 구분해서 나눈다. 

예외 처리

- 사용자가 잘못된 값을 입력한 경우 `throw`문을 사용해 예외를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.

- [X] 코치의 이름을 2글자 이상 4글자 미만이 아니면 예외 처리 한다.
  - e.g. ‘[ERROR] 코치는 최소 2명 이상 입력해야 합니다.’
- [X] 밥을 먹는 코치가 2명 이상 5명 이하가 아니면 예외 처리 한다.

코치 이름에 따라 못 먹는 메뉴

입력

- [X] 사용자에게 각 코치별 못 먹는 메뉴 입력을 받는다.
  - [X] 메시지: `${코치 이름}(이)가 못 먹는 메뉴를 입력해 주세요.'

내부기능

- [X] 못 먹는 음식은 컴마(,)로 구분한다.
...
- 사용자가 잘못된 값을 입력한 경우 `throw`문을 사용해 예외를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
- [X] 못 먹는 음식이 최소 0개 이하 또는 3개 이상인 경우 경우 예외 처리 한다.
